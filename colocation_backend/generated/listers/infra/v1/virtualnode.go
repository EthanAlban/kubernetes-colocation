...
type VirtualNodeLister interface {
	...
	List(selector labels.Selector) (ret []*v1.VirtualNode, err error)
...
// List lists all VirtualNodes in the indexer.
func (s *virtualNodeLister) List(selector labels.Selector) (ret []*v1.VirtualNode, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.VirtualNode))
	})
	return ret, err
}
...
type VirtualNodeNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.VirtualNode, error)
...
// List lists all VirtualNodes in the indexer for a given namespace.
func (s virtualNodeNamespaceLister) List(selector labels.Selector) (ret []*v1.VirtualNode, err error) {
...
// Get retrieves the VirtualNode from the indexer for a given namespace and name.
func (s virtualNodeNamespaceLister) Get(name string) (*v1.VirtualNode, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("virtualnode"), name)
	}
	return obj.(*v1.VirtualNode), nil
}
